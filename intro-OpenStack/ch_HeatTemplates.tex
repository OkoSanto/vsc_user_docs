...
\item Next, specify the required properties of each resource.
  ...
  \item When you open up the properties window for
    FloatingIPAssociation\_1, you can see that the \strong{Floating
      IP} and \strong{Port ID} properties are set correctly.  However,
    you still need to click \strong{\textsc{save}} in order to be able
...
\strong{Launch Stack} starts a wizard to create a stack from a
...
the button \strong{\textsc{create stack}}.

As an example, we can instantiate one of the examples from the \textsc{vsc}
